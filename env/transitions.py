...
def _parse_gdp_from_snippets(
    ...
    if len(values) == 1:
        return {"gdp_prev": round(values[0], 2), "gdp_curr": round(values[0] * 1.02, 2)}
    return None
...
def _build_hard_report(ticker: str, data: dict) -> str:
    ...
    lines += [
        ...
        f"The 7-day SMA of {sma7:.4f} was computed from recent closing prices." if isinstance(sma7, float) else "",
